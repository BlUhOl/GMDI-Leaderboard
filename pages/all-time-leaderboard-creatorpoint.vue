<script setup>
import { ALL_CP } from "@/composables/useSheet";

const headings = ref(null);
const result = ref([]);
const { data } = await ALL_CP();
...
<template>
  <title>All-Time Leaderboard - Creator Point</title>
  ...
  <div class="w-full h-full bg-background-950 px-2 xl:px-72 py-16">
    <div class="text-sm text-primary-600 breadcrumbs">
      <ul>
        ...
        <li class="cursor-default">All-Time Leaderboard - Creator Point</li>
      </ul>
    </div>

    <div class="my-2 mb-10">
      <h2 class="text-5xl text-primary-300 underline font-bold">
        All-Time Leaderboard - Creator Point
      </h2>
      <p class="text-primary-100 text-xl my-2">
        Top 50 players with the most Creator Points
      </p>
    </div>
      ...
      <table class="table table-xs md:table-lg">
        ...
        <tbody class="">
          ...
          >
            ...
            >
              {{ col[3] }} <img class="size-4 md:size-6" src="../assets/img/cp.png" />
            </td>
          </tr>
        </tbody>
      </table>
